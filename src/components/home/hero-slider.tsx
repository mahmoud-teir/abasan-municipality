...
export function HeroSlider({ slides, defaultTitle, defaultSubtitle, locale }: Props) {
    ...
    if (activeSlides.length === 0) {
        return (
            <section className="relative bg-gradient-to-br from-primary/90 to-primary py-20 md:py-32 overflow-hidden h-[600px] md:h-[700px] flex items-center justify-center">
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="relative z-10 container mx-auto px-4 h-full flex flex-col justify-center items-center text-center text-white">
                        ...
                        >
                            <h1 className="text-4xl md:text-6xl font-extrabold mb-6 tracking-tight leading-tight max-w-4xl drop-shadow-lg">
                                {title}
                            </h1>
                            {subtitle && (
                                <p className="text-xl md:text-2xl mb-10 opacity-90 font-light max-w-2xl mx-auto drop-shadow-md">
                                    {subtitle}
                                </p>
                            )}
                        </motion.div>
            ...
            {activeSlides.length > 1 && (
                <>
                    <Button
                        variant="ghost"
                        size="icon"
                        className="absolute left-4 top-1/2 -translate-y-1/2 text-white/50 hover:text-white hover:bg-white/10 rounded-full w-12 h-12"
                        onClick={prev}
                    >
                    ...
                    <Button
                        variant="ghost"
                        size="icon"
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-white/50 hover:text-white hover:bg-white/10 rounded-full w-12 h-12"
                        onClick={next}
                    >
